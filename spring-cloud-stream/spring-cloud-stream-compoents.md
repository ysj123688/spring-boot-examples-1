...
> 最新更新参考有道分享：http://note.youdao.com/noteshare?id=ac66e1649af231454820583d552a5228

参考：http://docs.spring.io/spring-cloud-stream/docs/current/reference/htmlsingle/

源码：https://github.com/Suzhida/spring-boot-examples/tree/master/spring-cloud-stream
...
2) @Input和@Output的注解指定的value值为binding的名称，具体的通道名称是有binding进行指定，==两者指定的binding的名称不能一样==；
...
@EnableBinding({BetGameStatusEventSource.class})
```

在Service注入使用
```
@Autowired
private BetGameStatusEventService betGameStatusEventService;
// 使用
betGameStatusEventService.publishEvent(null, new BetGameStatusEvent(id, dto.getStatus()));
```
